...
wget -O givers_chipi.js https://raw.githubusercontent.com/ChipiCoin/ChipiCoinGpuMiner/main/givers.js
wget -O givers_chipi.ts https://raw.githubusercontent.com/ChipiCoin/ChipiCoinGpuMiner/main/givers.ts
...
sed -i 's/require(".\/givers")/require(".\/givers_chipi")/g' send_chipi.js
sed -i 's/require(".\/givers")/require(".\/givers_chipi")/g' send_multigpu_chipi.js
...
if [ "\$1" = "gram" ]; then
    ...
    if [ "\$GPU_COUNT" = "1" ]; then
        CMD="node send_chipi.js --api \${API} --bin ./pow-miner-cuda --givers \${GIVERS} --timeout \${TIMEOUT}"
    else
...
echo -e "|                                                                        |"
echo -e "| Start mining with ${GREEN}./mine.sh mrdn/gram/chipi${NC}                            |"
